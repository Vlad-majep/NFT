...
import { Bid, NFT, ThirdwebSDK } from "@thirdweb-dev/sdk";
...
export default function TokenPage({ nft, contractMetadata }: Props) {
  ...
  const [timeToEnd, setTimeToEnd] = useState<number>();
  ...
  useEffect(() => {
    const getHighestBid = async () => {
      if (auctionListing && auctionListing[0]) {
        const bid = await marketplace?.englishAuctions.getWinningBid(
          auctionListing![0].id
        );
        const auction = await marketplace?.englishAuctions.getAuction(
          auctionListing![0].id
        );
        const timeToEnd =
          auction?.endTimeInSeconds! - new Date().getTime() / 1000;
        return {
          bid: bid?.bidAmountCurrencyValue.displayValue,
          time: timeToEnd,
        };
      }
    };
    ...
    getHighestBid()
      ...
      .then((data) => {
        console.log(data);
        setHighestBid(Number(data?.bid) || 0);
        setTimeToEnd(Number(data?.time) || 0);
      });
  });
            ...
            <Skeleton isLoaded={!loadingAuction}>
              {auctionListing && auctionListing[0] && (
                <Flex direction={"column"}>
                  ...
                  <Text>Auction will end up in {Math.floor(timeToEnd / 60)} minutes</Text>
                </Flex>
              )}
            </Skeleton>
          </Stack>
